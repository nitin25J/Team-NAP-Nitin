...
class HospitalModel(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "lng": self.lng,
            "coordinates": (
                {"lat": self.lat, "lng": self.lng} if self.lat and self.lng else None
            ),
        }
...
class EmergencyAlertModel(Base):
    ...
    type = Column(
        String
    )  # Flash Flood Advisory, High Water Warning, Heavy Rain Watch, etc.
...
class CitizenReportModel(Base):
    ...
    type = Column(
        String
    )  # Flooding, Bridge Damage, Trapped People, Shelter Full, Road Blocked
    location = Column(String)
    district = Column(String, index=True)
    description = Column(Text, nullable=True)
    status = Column(
        String, default="Pending Review"
    )  # Verified, Pending Review, Dispatched
    ...
    def to_dict(self):
        return {
            ...
            "type": self.type,
            "location": (
                f"{self.location}, {self.district}"
                if self.location
                and self.district
                and self.district not in self.location
                else (self.location or self.district)
            ),
            "district": self.district,
            "description": self.description or "",
            "severity": (
                "critical"
                if "critical" in (self.description or "").lower()
                or "flood" in (self.type or "").lower()
                else "moderate"
            ),
            "status": self.status or "Verified",
            "verified": self.media_attached
            or (self.status or "").lower() in ["verified", "dispatched", "en route"],
